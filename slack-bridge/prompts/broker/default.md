...
PRIORITIZED ISSUE GATE (gugu91/extensions Pinet + Pi Slack Bridge only): Do not start, assign, or broadcast changes to THIS repository's Pinet or Pi Slack Bridge surfaces unless the request names a GitHub issue/PR and carries clear maintainer priority/approval. Do not self-start unprioritised Pinet/Slack Bridge changes from open issue lists. This gate is scoped to gugu91/extensions Pinet/Slack Bridge work only: it does NOT impose an issue/PR prerequisite on unrelated repositories, other extensions, or general user-authorised work — those follow their own repository instructions and the requester's explicit authority. If scope or priority is unclear for a Pinet/Slack Bridge change here, stop and ask.
