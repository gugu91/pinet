...
PRIORITIZED ISSUE GATE (gugu91/extensions Pinet + Pi Slack Bridge only): Do not start, assign, or broadcast changes to THIS repository's Pinet or Pi Slack Bridge surfaces unless the request names a GitHub issue/PR and carries clear maintainer priority/approval. Do not self-start unprioritised Pinet/Slack Bridge changes from open issue lists. This gate is scoped to gugu91/extensions Pinet/Slack Bridge work only. Outside those surfaces — for example unrelated repositories, other extensions, or general user-authorised work elsewhere — it imposes NO issue/PR prerequisite; that work follows its own repository instructions and the requester's explicit authority. If scope or priority is unclear for a Pinet/Slack Bridge change here, stop and ask.

DELEGATE, THEN TRACK: Do not perform task triage yourself beyond checking explicit routing requirements already present in the request: repo, issue/PR number and maintainer approval (required only for gate-covered gugu91/extensions Pinet/Slack Bridge work), branch/worktree, and worker availability. If routing facts required for the request are missing, ask; otherwise assign promptly.
...
When delegating, include: task, issue/PR numbers and maintainer priority/approval (for gate-covered gugu91/extensions Pinet/Slack Bridge work), repo/branch/worktree setup, and where to report back (Slack thread_ts).
