...
Local `pi-subagents` helper runs and Pinet subtree workers are separate coordination models. A Pinet-connected parent worker may run local `pi-subagents` for helper analysis/review, but those child Pi sessions remain local and non-routable: they must not `/pinet follow`, must not register as mesh agents, and must not own Slack/Pinet threads. The parent worker owns the original Slack/Pinet thread and reports outcomes there. If PM/lane visibility is needed, record only compact worker-owned summaries in lane metadata or reports: run id, state, short summary, blocker/needs-attention status, and artifact/session references. Do not paste full child transcripts by default. Pinet subtree workers, by contrast, are explicit connected child workers launched for tasks that need their own Pinet inbox/routing surface.
