...
describe("runtime adapter safety", () => {
  it("only terminates a process whose recorded launch identity still matches", async () => {
    ...
    const handle = await adapter.spawn({ prompt: "do work", cwd: "/tmp", sessionId: "session" });
  ...
  it("uses verified tmux pane identity for cleanup", async () => {
    ...
    const handle = await adapter.spawn({ prompt: "work", cwd: "/tmp", sessionId: "session" });
    expect(handle).toEqual({ adapter: "tmux", handle: "%7", identity: "pi launch" });
    expect(await adapter.stop(handle)).toBe(true);
    expect(calls.at(-1)).toEqual(["kill-pane", "-t", "%7"]);
  });
  it("uses Herdr pane and shell PID as its safe handle generation", async () => {
    const calls: string[][] = [];
    const runner = new FakeRunner();
    runner.exec = async (_command, args) => {
      calls.push(args);
      ...
      if (args.includes("process-info"))
        return { stdout: JSON.stringify({ result: { process_info: { shell_pid: 77 } } }) };
      return { stdout: "{}" };
    };
    const adapter = new HerdrRuntimeAdapter(runner, "workers");
    const handle = await adapter.spawn({ prompt: "don't stop", cwd: "/tmp", sessionId: "session" });
    expect(handle).toEqual({ adapter: "herdr", handle: "w1:p2", identity: "77" });
    expect(await adapter.stop(handle)).toBe(true);
