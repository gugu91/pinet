...
Updates replace the complete Markdown body. Markdown is stored exactly as submitted, including leading/trailing whitespace; whitespace trimming is used only to reject empty documents. Concurrent updates are last-write-wins; timestamps are metadata, not revision guards. Deleting a project deletes its tasks. Search and list responses are bounded and paginated. The `projects`, `tasks`, and `search` tool actions accept `limit` (0–100) and `offset` (0–100000). Pass `externalChannel: null` to `project_update` to clear a channel link; omission preserves the current link.
