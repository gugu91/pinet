...
import { DEFAULT_SOCKET_PATH as PINET_DEFAULT_SOCKET_PATH } from "@pinet/broker-core/paths";
import { assertLoopbackTcpHost } from "@pinet/broker-core/raw-tcp-loopback";
import {
...
// agent-standards-ignore prefer-inline-single-use-helper: preserved verbatim from slack-bridge/broker/client.ts during the pinet-core extraction; keeps RPC error shaping as one named seam
...
// agent-standards-ignore prefer-inline-single-use-helper: preserved verbatim from slack-bridge/broker/client.ts during the pinet-core extraction; boundary parse of Node errno shapes
...
export type { BrokerClientTlsOptions };

export type BrokerConnectOpts =
  | { path: string }
  | {
      host: string;
      port: number;
      /**
       * Encrypted remote transport. Required for any non-loopback broker
       * endpoint; plaintext TCP remains loopback-only. Must configure an
       * explicit trust anchor (CA bundle and/or pinned certificate SHA-256).
       */
      tls?: BrokerClientTlsOptions;
    };
...
export class BrokerClient {
  private readonly connectOpts: { path: string } | { host: string; port: number };
  private readonly tlsOptions: BrokerClientTlsOptions | null;
  ...
  constructor(opts?: string | BrokerClientOptions) {
    ...
    if ("path" in opts) {
      ...
      assertBrokerClientTlsOptions(opts.tls, "broker client TLS connect target");
  ...
  private connectSocket(): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      const adopt = (sock: net.Socket): void => {
        this.socket = sock;
        this.connected = true;
        this.buffer = "";
        resolve();
      ...
      let sock: net.Socket;
      if (this.tlsOptions && "host" in this.connectOpts) {
        const tlsOptions = this.tlsOptions;
        const hasCa = typeof tlsOptions.ca === "string" && tlsOptions.ca.trim().length > 0;
        const tlsSock = tls.connect({
          host: this.connectOpts.host,
          port: this.connectOpts.port,
          // Pin-only trust performs its own leaf verification below; chain
          // verification is enforced whenever a CA bundle is configured.
          rejectUnauthorized: hasCa,
          ...(hasCa ? { ca: tlsOptions.ca } : {}),
        ...
        tlsSock.once("secureConnect", () => {
          const peerCertificate = tlsSock.getPeerCertificate();
          const failure = describeTlsPeerVerificationFailure(tlsOptions, {
            authorized: tlsSock.authorized,
            authorizationError: tlsSock.authorizationError
              ? String(tlsSock.authorizationError.message ?? tlsSock.authorizationError)
              : null,
            peerFingerprint256:
        ...
        sock = net.createConnection(this.connectOpts);
        sock.on("connect", () => {
          adopt(sock);
        });
      }
